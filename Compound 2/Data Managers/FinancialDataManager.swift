...
struct FinancialDataManager {
    ...
    static func listOfAllCompanies() -> [String] {
        var allCompanies = Bundle.main.paths(forResourcesOfType: "plist", inDirectory: financialStatementsSubdirectory)
    ...
    static func getSmartlabLinks() throws -> [String: String] {
        guard let plistPath = Bundle.main.path(forResource: "SmartlabLinks", ofType: "plist", inDirectory: quoteLinksSubdirectory) else {
            Logger.log(error: "FDM: unableToLoadPathAt \"Corporate Resources/QuoteLinks\"")
    ...
    static func getPublicCompanies() -> [String]? {
        return try? getSmartlabLinks().map(\.key)
    }
}
...
extension FinancialDataManager {
    static func getOperatingIncome(for company: String) -> (type: OperatingIncomeType, values: [(year: Int, value: Double)])? {
        ...
        guard let parsedData = parseData(from: &plistData, for: company) else { return nil }
    ...
    static func getNetIncome(for company: String) -> [FinancialFigure]? {
        guard var plistData = try? fetchPlistData(for: company) else {
            Logger.log(error: "Unable to load data for \(company)")
            return []
        }
    ...
    static func getDividends(for company: String) -> [FinancialFigure]? {
        guard var plistData = try? fetchPlistData(for: company) else {
            Logger.log(error: "Unable to load data for \(company)")
            return []
        }
        
        return parseData(from: &plistData, for: company)?.dividends
    }
}
...
extension FinancialDataManager {
    static func numberOfSharesFor(company: String) throws -> (numberOfOrdinaryShares: Int, numberOfPreferredShares: Int?) {
        ...
        if let numberOfOrdinaryShares = sharesIssued[company] {
            ...
            Logger.log(warning: "Unable to find the number of shares issued by \(company)")
            throw PlistParsingError.unableToFindNumberOfSharesIssued
        }
    }
    
    static func getMumberOfSharesForAllCompanies() throws -> [String: Int] {
        guard let plistPath = Bundle.main.path(forResource: "NumberOfSharesIssued", ofType: "plist", inDirectory: "Corporate Resources/NumberOfSharesIssued") else {
            Logger.log(error: "Unable to load the plist file containing the number of shares")
            throw PlistParsingError.unableToLoadDataAt("Corporate Resources/NumberOfSharesIssued")
        }
        ...
        var shareCounts: [String: Int] = [:]
        for shareCount in sharesIssued {
            shareCounts[shareCount.key] = (Int(truncating: shareCount.value))
        }
...
private extension FinancialDataManager {
    ...
    private static func parseData(from plist: inout Data, for company: String) -> FinancialIndicators? {
        guard let plistObject = try? PropertyListSerialization.propertyList(from: plist, options:PropertyListSerialization.ReadOptions(), format:nil) else {
            return nil
        }
        //Cast plistObject - If expected data type is Dictionary
        guard var plistDict = plistObject as? Dictionary<String, AnyObject> else {
            return nil
        }
    ...
    //Converting NSMutableArrays into Swift arrays contating tuples with financial indicators value
     static func convertIntoSwiftArray(_ objectiveCArray: NSMutableArray) -> [(year: Int, value: Double)] {
        ...
        for yearAndValue in objectiveCArray {
            ...
            let value = Double(swiftString.suffix(swiftString.count - 5)) ?? -1
...
extension FinancialDataManager {
    ...
    static let corporateDataDirectory = "Corporate Resources"
    
    static let quoteLinksSubdirectory = corporateDataDirectory + "/QuoteLinks"
    static let financialStatementsSubdirectory = corporateDataDirectory + "/FinancialStatements"
    static let ecosystemImagesSubdirectory = corporateDataDirectory + "/Ecosystems"
    static let numberOfSharesIssuedSubdirectory = corporateDataDirectory +  "/NumberOfSharesIssued"
    static let productionFiguresSubdirectory = corporateDataDirectory +  "/ProductionFigures"
    static let tradingViewLinksSubdirectory = corporateDataDirectory +  "/TradingViewLinks"
}
